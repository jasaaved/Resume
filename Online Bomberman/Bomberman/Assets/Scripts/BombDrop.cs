...
public class BombDrop : MonoBehaviour {
	...
	private void getInput(){
		...
		if(Input.GetKeyDown(KeyCode.Space)){
            ...
            if (GameObject.Find("Network").GetComponent<Network>().player_assignment == 1)
            {
                pos = GameObject.Find("Player1").transform.position;
                GameObject.Find("Player1").GetComponent<Player1Move>().player_info.bomb = true;
            }

            else if (GameObject.Find("Network").GetComponent<Network>().player_assignment == 2)
            {
            ...
            else if (GameObject.Find("Network").GetComponent<Network>().player_assignment == 3)
            {
