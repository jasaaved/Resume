...
using System.Collections.Generic;
    ...
    private void GetPerpendicularVector(ref Vector3 directionNormalized, out Vector3 side)
        ...
        private void Start()
        {
            ...
            start.x = transform.parent.position.x;
            start.y = transform.parent.position.y;
        start.z = transform.parent.position.z;

        end.x = transform.parent.position.x;
        end.y = transform.parent.position.y;
        end.z = transform.parent.position.z;
            grow = true;
            timer = 0.75f;
        }
        ...
        if (timer >= 0.0f)
        {
            if(!soundPlaying)
            {
                Instantiate<GameObject>(lightningSound);
            }
            soundPlaying = true;
