...
public class DKBossRoomScript : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.tag == "Player" && other.transform.name != "BasicShield" && other.transform.name != "ShieldThrow" && other.transform.name != "BasicShield(Clone)" && other.transform.name != "ShieldThrow(Clone)" && gamemanager.GetComponent<GameManager>().DK_key_found && !gamemanager.GetComponent<GameManager>().InCombat) //shield throw hotfix
        {
